...
Then, we make a density plot with a rugplot below to understand what we’re testing in a functional enrichment test.
...
To test if any co-expressed modules that we identify are associated with organismal phenotype data collected, we copy the trait data (upper lethal temperature) associated with each sample into WGCNA_TraitData.csv file. This can be found in:  cp/gpfs1/cl/ecogen/pbio6800/Transcriptomics/enrichmentYou can cp it to your /mydata directory.. 
